...
export const ResidenceCard: React.FC<ResidenceCardProps> = ({
  ...
  const primaryPhoneDigits = place.phone.split(/[/oO-]/)[0]?.replace(/\D/g, '') || place.phone.replace(/\D/g, '');
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-slate-600 text-sm mt-1 flex items-center gap-1.5 flex-wrap">
              ...
              <a
                href={`tel:${primaryPhoneDigits || cleanPhone}`}
                className="text-blue-700 hover:text-blue-900 font-semibold hover:underline decoration-blue-300 transition-colors inline-flex items-center gap-1 cursor-pointer"
                title={`Llamar directamente al ${place.phone}`}
              >
                <span>{place.phone}</span>
              </a>

              {primaryPhoneDigits && (
                <div className="no-print inline-flex items-center gap-1 ml-1.5">
                  <a
                    href={`https://wa.me/549${primaryPhoneDigits}`}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 px-2 py-0.5 rounded-lg transition-colors cursor-pointer"
            ...
            {/* Website / Enlace al sitio as direct link */}
            <div className="text-slate-600 text-sm mt-1 flex items-center gap-1.5 flex-wrap">
              ...
              {place.website && place.website.trim() ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-blue-700 hover:text-blue-900 font-medium hover:underline decoration-blue-300 max-w-[280px] sm:max-w-[420px] truncate transition-colors cursor-pointer"
                  title={`Abrir sitio web oficial: ${place.website}`}
                >
